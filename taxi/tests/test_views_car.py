...
class PrivateCarListViewTests(TestCase):
    def setUp(self) -> None:
        ...
        manufacturer2 = Manufacturer.objects.create(name="Honda", country="Japan")
        for car_number in range(number_of_cars):
            if car_number % 2 == 0:
                Car.objects.create(model=f"Model {car_number}", manufacturer=manufacturer1)
            else:
                Car.objects.create(model=f"Name {car_number}", manufacturer=manufacturer2)
