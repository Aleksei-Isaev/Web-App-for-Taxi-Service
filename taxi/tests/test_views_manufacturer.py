...
class PrivateManufacturerListViewTests(TestCase):
    def setUp(self) -> None:
        self.user = get_user_model().objects.create_user(username="test", password="test12345")
        self.client.force_login(self.user)
        number_of_manufacturers = 8
        for manufacturer_number in range(number_of_manufacturers):
            Manufacturer.objects.create(name=f"Name {manufacturer_number}", country=f"Country {manufacturer_number}")
    ...
    def test_pagination_is_five(self):
        ...
        self.assertEqual(list(response.context["manufacturer_list"]), list(manufacturers)[:5])
    ...
    def test_lists_all_manufacturers(self):
        ...
        self.assertEqual(list(response.context["manufacturer_list"]), list(manufacturers)[-3:])
        self.assertTrue("is_paginated" in response.context)
        self.assertTrue(response.context["is_paginated"])
